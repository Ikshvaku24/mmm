...
│   │   └── prior_builder.py  PRE-MODEL: generate a prior file from a vendor
│   │                         decomposition or a pillar/spend breakdown
│   ├── modelling/
...
│   │   ├── benchmark.py      the paste-your-benchmark sheet
...
│
├── samples/                  example input files you can copy
│   ├── benchmark_mapping_sample.csv
│   ├── vendor_contribution_sample.csv
│   └── pillar_spend_sample.csv
...
| generate a prior file from a vendor decomposition | `mmm/data/prior_builder.py` |
...
└── benchmark_contribution.csv    the vendor's numbers, canonicalised
```
...
| `METHODOLOGY.md` | the staged build, the prior ladder, how to check collinearity |
...
`../tests/run_all.py` — 1133 checks, ~10s, no PyMC needed. They import the
